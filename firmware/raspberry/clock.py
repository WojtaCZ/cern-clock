...
def firstSetupSequence():
    ...
    while not accelerometer.tapFlag:
        decoder.writeBannerSync("KLEPNI NA ME")
    ...
    while accelerometer.tapCounter < 2:
        if accelerometer.tapCounter == 0: 
            decoder.writeBannerSync("KLEPNI DVAKRAT")
    ...
    decoder.writeBannerSync("PRIPOJE SE NA NI A NASTAV ME")
...
def wakeUp():
    ...
    lastActivityTimestamp = ntp.localTime()

def shouldSleep(year, month, mday, hour, minute, second, weekday, yearday):
    global lastActivityTimestamp
    global SLEEP_ENABLED
    global SLEEP_TIMEOUT
    global SLEEP_TIMERANGE_ARRAY
